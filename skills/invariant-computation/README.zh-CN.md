...
```text
请帮我安装 `invariant-computation` skill，链接是：https://github.com/VeryMath/AI4Math-Computational-Mathematics.git。请读取包内 `SKILL.md`，安装其中声明的 Skill entrypoint，验证 `$invariant-computation` 可用，并告诉我是否需要重启 agent。
```
...
然后检查仓库文档是否指向包内 Skill，且没有新增本地绝对路径或私有 token。
